...
export default function RoomDetail() {
  ...
  useEffect(() => {
    if (!formData) {
      console.warn("Không có dữ liệu phòng học được truyền!");
    }
  }, [formData]);
  ...
  const handleChange = (
    ...
    setFormData((prev: Room) => ({ ...prev, [name]: value }));
  };

  if (!formData) {
    return <div>Không có dữ liệu phòng học.</div>;
  }

  const handleSave = async () => {
    try {
      const response = await fetch(
        `http://localhost:8080/phonghoc/update/${formData.id}?maphonghoc=${formData.id}`,
        {
          method: "PUT",
          headers: {
      ...
      alert("Đã xảy ra lỗi khi cập nhật thông tin phòng học!");
    }
  };
