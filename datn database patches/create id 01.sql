...
SELECT CONCAT('LV', LPAD(IFNULL(MAX(CAST(SUBSTRING(MALINHVUC, 3, 3) AS UNSIGNED)), 0) + 1, 3, '0')) 
FROM LINHVUCS;

-- XỬ LÝ LỚP HỌC - LOPHOCS

SELECT CONCAT('LH', LPAD(IFNULL(MAX(CAST(SUBSTRING(MALOPHOC, 3, 3) AS UNSIGNED)), 0) + 1, 3, '0')) 
FROM LOPHOCS; 
...
SELECT CONCAT('HV', LPAD(IFNULL(MAX(CAST(SUBSTRING(MAHOCVIEN, 3, 3) AS UNSIGNED)), 0) + 1, 3, '0')) 
FROM HOCVIENS; 

SELECT * FROM HOCVIENS;

-- XỬ LÝ THÍ SINH DỰ THI - THISINHDUTHIS

SELECT * FROM THISINHDUTHIS;
